"""
Entity Extraction for House Oversight DOJ Records Documents Only

Targets documents with data_set='house-oversight-doj' that don't have mentions yet.
Same scoped, merge-by-canonical-name pattern as extract_house_oversight_entities.py
(kept as a separate script rather than parameterizing, matching this project's
existing one-script-per-batch convention).
"""
...
def run_house_oversight_doj_extraction():
    """Run entity extraction on House Oversight DOJ Records documents only."""
    engine = get_engine()
    ...
    # Get House Oversight DOJ documents with text
    logger.info("Querying House Oversight DOJ documents...")

    docs = session.query(Document.id, DocumentText.full_text).join(
        DocumentText, Document.id == DocumentText.document_id
    ).filter(
        Document.data_set == 'house-oversight-doj',
    ...
    logger.info(f"Processing {total} House Oversight DOJ documents...")
    ...
    logger.info(f"\nTop 20 entities from House Oversight DOJ docs:")
    for e in top:
...
if __name__ == "__main__":
    run_house_oversight_doj_extraction()
